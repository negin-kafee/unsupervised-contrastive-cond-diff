...
def plot_bboxes_on_image(image, bboxes):
    ...
    bboxes: list of [y1, x1, y2, x2] or [top, left, bottom, right]
    """
    fig, ax = plt.subplots()
    ...
    plt.axis('off')
    plt.savefig('vol_with_bboxes.png')

def get_bounding_boxes(mask):
    mask_np = mask.cpu().numpy().astype(np.uint8)
    labeled = label(mask_np, connectivity=1)
    props = regionprops(labeled)
    
    bboxes = []
    for prop in props:
        min_row, min_col, max_row, max_col = prop.bbox
        bboxes.append([min_row, min_col, max_row, max_col])  # (y1, x1, y2, x2)
    
    return bboxes

# NOTE: NOVA benchmark evaluation
def _predict_step(self, final_volume, data_orig, batch_idx, ID):
    ...
    # move data to CPU
    diff_volume = diff_volume.cpu() # [1, 1, 240, 240]
    data_orig = data_orig.cpu() # [1, 1, 240, 240]
    final_volume = final_volume.cpu() # [240, 240]

    # Filter the DifferenceImage
    if self.cfg['medianFiltering']:
        diff_volume = torch.from_numpy(apply_2d_median_filter(diff_volume.numpy().squeeze(),kernelsize=self.cfg.get('kernelsize_median',5))).unsqueeze(0) # bring back to tensor

    # Binarize diff_volume
    diff_volume_binarized = (diff_volume > 0.5).int()
    
    # Fit bounding boxes on the salient regions
    bboxes = get_bounding_boxes(diff_volume_binarized.squeeze())
    
    # Save point coordinates into a dict: {"img_ID": points}
    self.bboxes_dict[ID[0]] = bboxes
...
def _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label_vol) :
       ...
       # Calculate Reconstruction errors with respect to anomal/normal regions
        ...
        #print(f"[INFO]: {diff_volume.shape}")
        #print(diff_volume)
        #print(f"[INFO]: {data_mask.shape}") # OK
        #print(data_mask) # OK
        ...
        if self.cfg.evalSeg and self.dataset[0] not in self.healthy_sets: # only compute metrics if segmentation is available
            ...
            #if ID[0] in ["BraTS2021_00011", "BraTS2021_00062"]:
            #    breakpoint()
